    ...
    <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center relative overflow-hidden"
      ...
      >
        ...
        >
          ...
          {/*
           * Apple SSO — full-width to mirror the Google button above.
           * Facebook SSO was removed April 2026 (we're consolidating to
           * Google + Apple + email/password for the mobile launch).
           * Still wired into Supabase: signInWithApple → auth.signInWithOAuth({ provider: 'apple' })
           */}
          <button
            onClick={() => wrapBusy(signInWithApple)}
            disabled={disabled}
            className="w-full h-13 rounded-2xl flex items-center justify-center gap-3 font-semibold text-sm bg-black text-white hover:bg-gray-900 transition-all shadow-lg shadow-black/20 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
            style={{ height: '52px' }}
          >
            {busy ? (
