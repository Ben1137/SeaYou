...
const IMPACT_STYLES: Record<ScoreFactor['impact'], { text: string; bg: string; Icon: typeof TrendingUp }> = {
  positive: { text: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-400/30', Icon: TrendingUp },
  neutral:  { text: 'text-white/60',    bg: 'bg-white/5 border-white/10',              Icon: Minus },
  negative: { text: 'text-rose-400',    bg: 'bg-rose-500/10 border-rose-400/30',       Icon: TrendingDown },
};
...
export const ScoreBreakdownModal: React.FC<ScoreBreakdownModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-0 sm:p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="w-full sm:max-w-md max-h-[90vh] overflow-y-auto bg-gradient-to-b from-slate-900 to-slate-950 border border-white/10 rounded-t-2xl sm:rounded-2xl shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div className="flex items-baseline gap-3">
            <h2 id="score-breakdown-title" className="text-lg font-bold text-white">
              {activityLabel}
            </h2>
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full glass-inner flex items-center justify-center hover:bg-white/10 transition"
        ...
        <div className="px-4 pt-4">
          <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${score.color} bg-white/5 border border-white/10`}>
        ...
        <div className="p-4 space-y-2">
          <h3 className="text-[10px] font-bold tracking-widest text-white/50 uppercase mb-2">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className="text-sm text-white/90 truncate">{factor.label}</span>
                </div>
          ...
          {score.warnings.length > 0 && (
            <div className="mt-3 p-3 rounded-lg bg-amber-500/10 border border-amber-400/30">
              <h4 className="text-[10px] font-bold tracking-widest text-amber-300 uppercase mb-1">
      ...
      {toast && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[10000] px-4 py-2.5 bg-slate-800 border border-amber-400/40 rounded-xl shadow-lg text-sm text-white max-w-[90%] text-center animate-fade-in"
          role="status"
        >
          🔒 {toast}
        </div>
