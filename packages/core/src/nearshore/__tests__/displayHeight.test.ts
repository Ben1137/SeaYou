...
import { brekerDisplayFactor } from '../displayHeight';

describe('breakerDisplayFactor', () => {
  it('should return 1.0 for long-period swell (T ≥ 14s)', () => {
    expect(brekerDisplayFactor({ T: 14 })).toBeCloseTo(1.0, 2);
    expect(brekerDisplayFactor({ T: 16 })).toBeCloseTo(1.0, 2);
    expect(brekerDisplayFactor({ T: 20 })).toBeCloseTo(1.0, 2);
  });

  it('should return ~0.35 for very short chop (T ≤ 4s)', () => {
    expect(brekerDisplayFactor({ T: 4 })).toBeLessThan(0.36);
    expect(brekerDisplayFactor({ T: 3 })).toBeLessThan(0.36);
    expect(brekerDisplayFactor({ T: 1 })).toBeLessThan(0.36);
  });

  it('should transition smoothly from short to long period', () => {
    const T4 = brekerDisplayFactor({ T: 4 });
    const T8 = brekerDisplayFactor({ T: 8 });
    const T14 = brekerDisplayFactor({ T: 14 });
  ...
  it('should damp short-period wind-swell (T=5-8s) to realistic face heights', () => {
    ...
    const factor_5s = brekerDisplayFactor({ T: 5.2 });
  ...
  it('should clamp output to [0, 1]', () => {
    expect(brekerDisplayFactor({ T: -5 })).toBeLessThanOrEqual(1);
    expect(brekerDisplayFactor({ T: -5 })).toBeGreaterThanOrEqual(0);

    expect(brekerDisplayFactor({ T: 0 })).toBeLessThanOrEqual(1);
    expect(brekerDisplayFactor({ T: 0 })).toBeGreaterThanOrEqual(0);

    expect(brekerDisplayFactor({ T: 100 })).toBeLessThanOrEqual(1);
    expect(brekerDisplayFactor({ T: 100 })).toBeGreaterThanOrEqual(0);
  });

  it('should handle NaN/Infinity gracefully', () => {
    expect(brekerDisplayFactor({ T: NaN })).toBe(1.0);
    expect(brekerDisplayFactor({ T: Infinity })).toBe(1.0);
    expect(brekerDisplayFactor({ T: -Infinity })).toBe(1.0);
  });

  it('should return higher factors for longer periods (monotonic)', () => {
    const periods = [4, 5, 6, 7, 8, 10, 12, 14, 16];
    const factors = periods.map(T => brekerDisplayFactor({ T }));
  ...
  it('should show factor ~0.8 for T=10s (medium swell)', () => {
    const factor = brekerDisplayFactor({ T: 10 });
  ...
  it('should show factor ~0.5 for T=6.5s (windswell transition)', () => {
    const factor = brekerDisplayFactor({ T: 6.5 });
